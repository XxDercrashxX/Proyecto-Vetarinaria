...
if ($_POST) {
    ...
    $resultado = $citaController->crearCita($datos);
    
    if ($resultado['success']) {
        header("Location: citas.php?success=create");
        exit();
    } else {
...
<html lang="es">
  <head>
    ...
    <title>Crear Cita - VETEICA CLINIC</title>
    <link
      rel="stylesheet"
    ...
    <main class="container">
      ...
      <div class="create-form-container">
        <form method="POST" action="">
            ...
            <select name="id_paciente" required>
              ...
              <?php while ($pacienteRow = $stmtPacientes->fetch(PDO::FETCH_ASSOC)): ?>
                <option value="<?php echo $pacienteRow['id']; ?>" 
                        <?php echo (($_POST['id_paciente'] ?? '') == $pacienteRow['id']) ? 'selected' : ''; ?>>
          ...
          <div class="form-group-create">
            <label>Fecha:</label>
            <input type="date" name="fecha" value="<?php echo htmlspecialchars($_POST['fecha'] ?? ''); ?>" required 
          ...
          <div class="form-group-create">
            <label>Motivo:</label>
            <input type="text" name="motivo" value="<?php echo htmlspecialchars($_POST['motivo'] ?? ''); ?>" required />
          </div>
          ...
          <div class="form-group-create">
            <label>Estado:</label>
            <select name="estado">
              <option value="programada" <?php echo (($_POST['estado'] ?? 'programada') == 'programada') ? 'selected' : ''; ?>>Programada</option>
              <option value="completada" <?php echo (($_POST['estado'] ?? 'programada') == 'completada') ? 'selected' : ''; ?>>Completada</option>
              <option value="cancelada" <?php echo (($_POST['estado'] ?? 'programada') == 'cancelada') ? 'selected' : ''; ?>>Cancelada</option>
            </select>
          </div>
